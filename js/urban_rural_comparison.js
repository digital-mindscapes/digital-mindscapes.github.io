...
let ruccGroupMode = false;
let ruccFilter = "All"; // "All", "Metro", "Nonmetro"
...
function updateMapData() {
    ...
    geoFeatures.forEach(feature => {
        ...
        let fips = null;
        if (pId && pId.length === 5 && !isNaN(pId)) {
            fips = pId;
        } else if (county && county.GEOID) {
            fips = county.GEOID;
        } else if (county && county.fips) {
            fips = county.fips;
        }

        let rucc_class = (fips && globalRuccData[fips]) ? globalRuccData[fips].classification : (county && county.rucc_class ? county.rucc_class : "Unknown");

        countyDataLookup[pId] = county ? { ...county, rucc_class } : { name: name, state: stateAbbr, rucc_class: rucc_class }; // Store for chart
        ...
        // Apply RUCC Filter to Map View
        let isFilteredOut = false;
        if (ruccFilter !== "All" && rucc_class !== ruccFilter) {
            isFilteredOut = true;
            val = null; // Blank it out from the heat map gradient
        }
...
function initMap() {
    ...
    mapPolygonSeries.mapPolygons.template.adapters.add("tooltipText", function (text, target) {
        ...
        if (dataItem) {
            ...
            if (dataContext && dataContext.isFilteredOut) {
                return "{name} ({rucc_class}): Filtered Out";
            }
            ...
            return "{name}: [bold]{value}" + (isPercent ? "%" : "") + "[/] ({rucc_class})";
        }
        return text;
    });
    ...
    mapPolygonSeries.mapPolygons.template.events.on("pointerover", function (ev) {
        ...
        if (dataItem && dataItem.dataContext && dataItem.dataContext.isFilteredOut) {
    ...
    mapPolygonSeries.mapPolygons.template.adapters.add("fill", function (fill, target) {
        ...
        if (!heatmapMode) {
            if (dataContext && dataContext.rucc_class) {
                // If filter is active, only color matches. If all, color all according to their class
                if (ruccFilter === "All" || dataContext.rucc_class === ruccFilter) {
                    if (ruccColors && ruccColors[dataContext.rucc_class]) {
                        return am5.color(ruccColors[dataContext.rucc_class]);
                    }
                }
            }
        } else {
    ...
    mapPolygonSeries.mapPolygons.template.events.on("click", function (ev) {
        ...
        if (dataItem.dataContext && dataItem.dataContext.isFilteredOut) return;
...
function initChart() {
    ...
    barSeries.columns.template.adapters.add("fill", function (fill, target) {
        if (ruccGroupMode) {
            const dataContext = target.dataItem.dataContext;
            if (dataContext && dataContext.rucc_class && ruccColors[dataContext.rucc_class]) {
                return am5.color(ruccColors[dataContext.rucc_class]);
            }
            return am5.color(0xc83830);
        }
...
function updateChart() {
    ...
    const data = [];
    const ruccOrder = ["Metro", "Nonmetro"];
    const selectedByRucc = { "Metro": [], "Nonmetro": [] };

    selectedCounties.forEach(id => {
        const countyData = countyDataLookup[id];

        if (countyData) {
            let rc = countyData.rucc_class || "Unknown";
            ...
            const item = { name: shortName, value: val, rucc_class: rc };
            if (rc === "Metro" || rc === "Nonmetro") {
                selectedByRucc[rc].push(item);
            } else {
                selectedByRucc["Nonmetro"].push(item);
            }
        }
    });
    ...
    if (ruccGroupMode) {
        legend.show();
        ruccOrder.forEach(rclass => {
            selectedByRucc[rclass].sort((a, b) => a.value - b.value);
            selectedByRucc[rclass].forEach(item => data.push(item));
        });
    } else {
        legend.hide();
        let allItems = [];
        ruccOrder.forEach(rclass => {
            if (selectedByRucc[rclass]) {
                selectedByRucc[rclass].forEach(item => allItems.push(item));
            }
        });
...
function updateChartExtras(selectedByRucc) {
    ...
    if (ruccGroupMode) {
        const ruccOrder = ["Metro", "Nonmetro"];
        ruccOrder.forEach(rc => {
            const counties = selectedByRucc[rc];
            if (counties.length > 0) {
                const lastCounty = counties[counties.length - 1].name;
                const color = am5.color(ruccColors[rc]);
                ...
                if (verticalChartMode) {
                    rangeDataItem.get("label").setAll({
                        fill: color, text: rc, location: 1, fontWeight: "bold", dy: 80, rotation: 0, centerX: am5.p50
                    });
                    ...
                    rangeDataItem.get("label").setAll({
                        fill: color, text: rc, location: 1, fontWeight: "bold", dx: -130
                    });
                    rangeDataItem.get("grid").setAll({ stroke: color, strokeOpacity: 1, location: 1 });
                    rangeDataItem.get("tick").setAll({
                ...
                legendData.push({ name: rc, color: color });
            }
        });
    }
